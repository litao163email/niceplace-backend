...
import com.taoli.niceplace.model.request.UserLoginRequest;
import com.taoli.niceplace.model.request.UserRegisterRequest;
...
public class UserController {
    ...
    public BaseResponse<List<User>> matchUsers(long num, HttpServletRequest request) {
        ...
        Gson gson = new Gson();
        //tagList:用户自己的标签不设置时返回推荐的数据
        List<String> tagList = gson.fromJson(tags, new TypeToken<List<String>>() {
        }.getType());
        if (  CollectionUtils.isEmpty(tagList) || tagList.size()==0){
            return ResultUtils.success(this.recommendUsers(num,1,request).getData().getRecords());
        }
        //用户自己的标签设置时返回的数据
